...
pdf("Zalewski_problem1.pdf", height = 7, width = 7)

heatmap.2(edata,
          main = "Bottomly Data Heatmap",
          notecol = "black",
          density.info = "none",
          trace = "none",
          margins = c(12, 9),
...
top5_loadings <- as.data.table(svd.out$v[, 1:5])

melted_loadings <- melt(top5_loadings, variable.name = "Vector", value.name = "Loading")


ggplot(melted_loadings) +
  geom_violin(aes(x = factor(Vector), y = Loading, fill = factor(Vector)), scale = "width", draw_quantiles = c(0.25, 0.5, 0.75)) +
  labs(title = "Violin plots of the top 5 left singular vectors") +
  xlab("Left Singular Vectors") +
  ylab("Loading") +
  theme_minimal()

pdf("Zalewski_problem4.pdf")
print(last_plot())
dev.off()
...
kmeans_clusters <- kmeans(edata, centers = 5)

set.seed(1)
tsne_out <- Rtsne(edata, pca = FALSE, perplexity = 60)
tsne_out <- data.table(tsne_out$Y)

tsne_clusters <- cbind(tsne_out, Cluster = as.factor(kmeans_clusters$cluster))

tsne_plot <- ggplot(tsne_clusters) + 
  geom_point(aes(x = V1, y = V2, col = Cluster)) +
  labs(title = "t-SNE Projection with K-means Clusters",
       x = "t-SNE Dimension 1", y = "t-SNE Dimension 2",
       color = "Cluster") +
  theme_minimal()

pdf("Zalewski_problem5.pdf")
print(tsne_plot)
dev.off()
